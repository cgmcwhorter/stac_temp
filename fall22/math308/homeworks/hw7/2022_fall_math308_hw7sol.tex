...
%\sol 
%\begin{enumerate}[(a)]
%\item The relation is a function. For each input $x$, there is precisely one possible value for $y$---namely, the one obtained by evaluating $x^2 + 5$ at the given input $x$. The image is the set $\{ y \colon y= x^2 + 5, x \in \mathbb{Z} \}$, i.e. the relation $\{ (x, y) \colon x, y \in \mathbb{Z}, y= x^2 + 5 \}$. 
%
%\item 
%\item 
%\item 
