...
% Fix an integer $n > 1$ and let $\{ a_n \}_{n \in \mathbb{N}}$ and $\{ b_n \}_{n \in \mathbb{N}}$ be sequences. Then $\displaystyle \prod_{k=1}^n a_n b_n= \prod_{k=1}^n a_n \cdot \prod_{k=1}^n b_n$ but $\displaystyle \sum_{k=1}^n a_n b_n \neq \sum_{k=1}^n a_n \cdot \sum_{k=1}^n b_n$. 

% Suppose that $a_{n+2}= 6a_n - a_{n+1}$ with $a_0= 4$ and $a_1= 3$. Then the characteristic polynomial is given by the equation $x^2= 6 - x$, i.e. the characteristic polynomial is $x^2 + x - 6$. Because $x^2 + x - 6= (x + 3)(x - 2)$ has roots $-3, 2$, the general solution is $a_n= c_1(-3)^n + c_2 2^n$. The specific solution is then $a_n= (-3)^n + 3 \cdot 2^n$. 

% $6^{2022} \equiv 1 \mmod 5$

%There is a unique solution to the following system of linear congruences:
%	\[
%	\begin{aligned}
%	2x - 1 &\equiv 5 \text{ mod } 3 \\
%	x &\equiv 0 \text{ mod } 5 \\
%	6x &\equiv 5 \text{ mod } 7
%	\end{aligned}
%	\]
