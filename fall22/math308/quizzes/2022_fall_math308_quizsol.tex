...
% If $A$ and $B$ are events, then $P(A \cap B)= P(A) \cdot P(B)$. 



%Let $G$ be a graph. Then adjacency in $G$ is transitive; that is, if $v_1, v_2, v_3 \in V(G)$ with $v_1$ adjacent to $v_2$ and $v_2$ adjacent to $v_3$, then $v_1$ is adjacent to $v_3$. 



% Any graph such that every vertex has even degree has an Euler circuit. 

% Every complete graph with more than one vertex has a Hamiltonian circuit. 
